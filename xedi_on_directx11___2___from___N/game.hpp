...
public :
	...
	void PreparingDraw() {
		...
		unsigned char* vs_ba = GetByteArrayFromFile("Simple_VS_02.cso",vs_ba_len);
		assert( vs_ba != NULL );

		unsigned int ps_ba_len=0;
		unsigned char* ps_ba = GetByteArrayFromFile("Simple_PS_02.cso",ps_ba_len);
		assert( ps_ba != NULL );
		...
		//load texture 
		CreateDDSTextureFromFile( m_Device , L"../data/textures/texture_02.DDS", nullptr, &m_TextureShaderResView, MAXSIZE_T);
		assert ( m_TextureShaderResView );
	}
	void Update(const double delta_time) {
		XMVECTOR eye = XMVectorSet(0.0f, 0.0f, m_AdjustableValues[3], 0.0f);
	...
	void OnHandleInput(int pid,int px,int py) {
		...
		case 76: // 'l'
			...
			break;
		case 78: // 'n'
			m_AdjustableValues[3] +=0.1f; // zoom in cam
			if( m_AdjustableValues[3] > 10 )
				m_AdjustableValues[3] = 0;
			break;
		case 77: // 'm'
			m_AdjustableValues[3] -=0.1f; // zoom out cam
			if( m_AdjustableValues[3] < 0 )
				m_AdjustableValues[3] = 10;
			break;
		default:
			break;
		};
